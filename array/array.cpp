...
public:
    ...
    void add_array(pool_base &pop, const char *name, int size)
    {
        if (find_array(name) != nullptr) {
            ...
            std::cout << "If you prefer, you can reallocate this arrary " << std::endl;
    ...
    {
        // prevArr will equal head if array wanted is either first OR second element
        ...
        if (strcmp(prev_arr->name, name) == 0) {
            // cur = prev= head, either only one element in list or array is first element
            cur_arr = head;
        } else {
    ...
    void print_usage(array_op op, std::string arg_zero)
    {
        switch (op) {
            case array_op::PRINT:
                std::cerr << "print array usage: " << arg_zero << " <file_name> print <array_name>" << std::endl;
                break;
            case array_op::FREE:
                std::cerr << "free array usage: " << arg_zero << " <file_name> free  <array_name>" << std::endl;
                break;
            case array_op::REALLOC:
                std::cerr << "realloc array usage: " << arg_zero << " <file_name> realloc <array_name> <size>" << std::endl;
                break;
            case array_op::ALLOC:
                std::cerr << "alloc array usage: " << arg_zero << " <file_name> alloc <array_name> <size>" << std::endl;
                break;
            default:
                std::cerr << "usage: " << arg_zero << " <file_name> <print|alloc|free|realloc> <array_name>" << std::endl;
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4) {
        std::cerr << "usage: " << prog_name
                  << " <file_name> <print|alloc|free|realloc> <array_name>" << std::endl;
        return 1;
    }
    ...
    if (strlen(name) > MAX_BUFFLEN) {
        std::cout << "Name exceeds buffer length of 30 characters. Please shorten and try again." << std::endl;
        return 1;
    }
    ...
    if (file_exists(file) != 0)
        pop = pool<examples::pmem_array>::create(file, LAYOUT, POOLSIZE, CREATE_MODE_RW);
    else
